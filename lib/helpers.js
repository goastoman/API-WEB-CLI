    ...
    if(templateName){
        ...
        fs.readFile(templatesDir + templateName + '.html', 'utf8', function(err, str){
            if(!err && str && str.length > 0){
                //do interpolation to the string
...
//add the universal header and footer to a string a pass provider data object for interpolation
helpers.addUniversalTemplates = function(str, data, callback){
    ...
    helpers.getTemplate('_header', data, function(err, headerString){
        if(!err && headerString){
            ...
            callback('Could not fint the header template')
        };
    });
};
...
helpers.interpolate = function(str, data){
    ...
    //add the templsteGlobals do the data object prepending their key name with Global
...
helpers.getStaticAsset = function(fileName, callback){
    ...
    if(fileName){
        ...
        callback('A valied file name was not specified');
    };
};
