...
workers.validateCheckData = function(originalCheckData){
    ...
    //if all the checks pass, pass the data along to the next steps in the process
...
//perform the check, send the originalCheckData and the outcome of the checl process, to the next step in the process
workers.performCheck = function(originalCheckData){
    ...
    //parse the hostename and the path out of the original check data
...
//process the check outcome update the check data as needed trigger an alers if needed
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome){
    ...
    _data.update('checks', newCheckData.id, newCheckData, function(err){
        if(!err){ 
            ...
            debug("Error trying to save iupdates to one of the checks");
        };
    });
};
    ...
    _logs.list(false, function(err, logs){
        if(!err && logs && logs.length > 0){
            logs.forEach(function(logName){
                //cmpress the data to a different file
                ...
                _logs.compress(logId, newFileId, function(err){
                    if(!err){
                        ...
                        debug('Error: compresing one of the log files', err);
                    };
                });
            });
        } else {
...
//timer to execute log-rotation one per day
...
workers.init = function(){
    ...
    //compross all the logs immdiately
    workers.rotateLogs();

    //call the compression loop for compressing logs
